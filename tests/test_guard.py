...
def test_block_by_count():
    ...
    guard = RiskGuard(acc)
    assert not guard.allow_new_position("BTCUSDT", 1)


def test_block_by_risk():
    acc = NS(equity_usd=10000, open_positions=[NS(risk_pct=5)] * 3)
    guard = RiskGuard(acc)
    assert not guard.allow_new_position("ETHUSDT", 6)
...
def test_dd_lock():
    ...
    assert not g.allow_new_position("BTCUSDT", 1)
...
def test_manager_guard_blocks(caplog):
    ...
    async def maybe_open(engine, pct):
        if guard.allow_new_position("TEST", pct):
